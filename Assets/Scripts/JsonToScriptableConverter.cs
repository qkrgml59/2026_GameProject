...
{
    public int? id;            //int?는 Nullable<int>의 축약표현
...
public class JsonToScriptableConverter : EditorWindow
{
    ...
    private ConversoinType conversionType = ConversoinType.Items;
    ...
    void OnGUI()
    {
        ...
        //변환 타입 선택
        conversionType = (ConversoinType)EditorGUILayout.EnumPopup("Conversion Type : ", conversionType);

        //타입에 따라 기본 출력 폴더 설정
        if (conversionType == ConversoinType.Items && outputFolder == "Assets/ScriptableObjects")
        {
            outputFolder = "Assests/Scriptable/Items";
        }
        else if (conversionType == ConversoinType.Items && outputFolder == "Assets/ScriptableObjects")
        {
            outputFolder = "Assests/ScriptableObejcts/Dialog";
        }
        ...
        if(GUILayout.Button("Conver to Scriptable Ojbects"))
        {
            ...
            {
                case ConversoinType.Items:
                    ConvertJsonToItemScriptableObjects();
                    break;
                case ConversoinType.Dialogs:
    ...
    private void ConvertJsonToItemScriptableObjects()                          //JSON 파일을 ScriptableObject 파일로 변환시켜주느 함수
    {
        //폴더 생성
    ...
    private void ConvertJsonToDialogScriptableObjects()
    {
        ...
        try
        {
            ...
            foreach(var rowData in rowDataList)
            {
                ...
                dialogSO.choices = new List<DialogChoicesSO>();
                //초상화 로드 (경로가 있을 경우)
            ...
            foreach (var rowData in rowDataList)
            {
                ...
                if(!rowData.id.HasValue && !string.IsNullOrEmpty(rowData.choiceText )&& rowData.choicenextId.HasValue)
                {
                    ...
                    //이 선태지 바로 위에 있는 대화Id가 있는 항목을 찾음
                    ...
                    {
                        DialogChoicesSO choiceSO = ScriptableObject.CreateInstance<DialogChoicesSO>();
